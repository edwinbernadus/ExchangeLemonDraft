...
namespace ExchangeLemonCore.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            returnUrl = EnsureReturnUrlSpotMarket(returnUrl);
        ...
        private string EnsureReturnUrlSpotMarket(string returnUrl)
        {
            if (string.IsNullOrEmpty(returnUrl))
            {
                ...
                {
                    if (referer.Contains("SpotMarket/Details/"))
                    {
                        var uri = new Uri(referer);
                        var output = uri.AbsolutePath;
                        return output;
                    }
                }
            }
        ...
        public async Task<IActionResult> Login(LoginViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    _ = await repoUser.GetOrPopulateUserProfile(model.Email);
                    _logger.LogInformation("User logged in.");
                    if (string.IsNullOrEmpty(returnUrl))
                    {
                        // return RedirectToAction("index", "spotMarket");
            ...
            else
            {
                ...
                if (result2.Succeeded)
                {
                    ...
                    if (result2.Succeeded)
                    {
                        ...
                        var user2 = await businessLogic.GetOrPopulateUserProfile(email);
                        await FakeAccountService.InitFund(user2,db);
                        
                        var returnUrl2 = "/";
                        return RedirectToLocal(returnUrl2);
                    }
                }
                else
                {
                    return Content("no data login - 2");
                }
                return Content("no data login - 3");
            }
            return Content("no data login");
        }
