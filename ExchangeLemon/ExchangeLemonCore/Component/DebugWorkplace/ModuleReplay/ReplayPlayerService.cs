...
namespace DebugWorkplace
{
    ...
    //: IReplayPlayerService
    {
        ...
        public async Task<List<LogItemEventSource>> GetReplayItemsAsync()
        {
            ...
            var items = await this.SourceContext.LogItemEventSources
                .Where(x => methodNames.Contains(x.MethodName) && x.IsRequest)
                .OrderBy(x => x.Id).ToListAsync();

            var itemsRespond = await this.SourceContext.LogItemEventSources
                .Where(x => methodNames.Contains(x.MethodName) && x.IsRequest == false && x.ExceptionMessage == null)
                .OrderBy(x => x.Id).ToListAsync();

            var itemsRespondNotNull = await this.SourceContext.LogItemEventSources
               .Where(x => methodNames.Contains(x.MethodName) && x.IsRequest == false && x.ExceptionMessage != null)
               .OrderBy(x => x.Id).ToListAsync();


            var total1 = items.Count();
            var total2 = itemsRespond.Count();
            var total3 = itemsRespondNotNull.Count();

            var result = items.Join(itemsRespond, x => x.SessionId, x => x.SessionId, (x, y) => x).ToList();
            var result2 = result.OrderBy(x => x.Id).ToList();
            return result2;
        }
        ...
        private List<string> GetMethodNames()
        {
            ...
            methodNames.Add(typeof(AddExternalManuallyCommand).Name);
