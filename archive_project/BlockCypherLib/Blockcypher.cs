...
namespace BlockCypher {
    public class Blockcypher {
        ...
        public async Task<string> PushRawTransaction(string transactionId,string rawContent)
        {
            //var output = await GetAsync<InquiryTransactionResult>(string.Format("txs/{0}", transactionId));
            //return output;

            var input = string.Format("txs/{0}", transactionId);
            var m = await PostAsyncPlain(input, rawContent) ;
            return m;
        }


        
        public async Task<HookInfo> GenerateHookSendTransferConfirmation(string address, string url,int totalConfirmations)
        {
            try
            {
                //string evt = "tx-confirmation";
                var evt = "unconfirmed-tx";

               var m = await PostAsync<HookInfo>($"hooks?token={this.UserToken}", new
               {
                   @event = evt,
                   url,
                   address,
            ...
            {
                return await Task.Factory.StartNew(() => new HookInfo
                {
        ...
        public Task<HookInfo> GenerateHook(string address, HookEvent hook, string url) {
            try {
                ...
                return PostAsync<HookInfo>($"hooks?token={this.UserToken}", new {
                    @event = evt,
                    url,
                    address
                });
        ...
        public async Task<List<HookInfo>> GetListHook()
        {
            try
            {
                
                var output =  await GetAsync<List<HookInfo>>($"hooks?token={this.UserToken}");
                return output;
            }
            catch
            {
                return Task.Factory.StartNew(() => new List<HookInfo>()).Result;
            }
        }

        public async Task<HookInfo> InquiryHook(string id)
        {
            try
            {

                var output = await GetAsync<HookInfo>($"hooks/{id}?token={this.UserToken}");
                return output;
            }
            catch
            {
        ...
        public async Task<bool> DeleteHook(string id)
        {
            try
            {

                var output = await DeleteAsync<string>($"hooks/{id}?token={this.UserToken}");
                return true;
            }
            catch
            {
        ...
        public async Task<InquiryTransactionResult> InquiryTransaction(string transactionId)
        {
            var output = await GetAsync<InquiryTransactionResult>(string.Format("txs/{0}", transactionId));
            return output;
        }
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        internal async Task<T> DeleteAsync<T>(string url)
        {
            ...
            var response = await client.DeleteAsync(string.Format("{0}/{1}", BaseUrl, url));
        ...
        internal async Task<string> PostAsyncPlain (string url, string requestJson) 
        {
            var client = GetClient();
